    ...
    if [ -f "/data/.pre_db_created" ]; then
        ...
        if [ -n "${ADMIN_USER}" ]; then
          ...
          do
              ...
              influx -host=${INFLUX_HOST} -port=${INFLUX_API_PORT} -username=${ADMIN_USER} -password="${PASS}" -execute="CREATE RETENTION POLICY four_weeks ON ${x} DURATION 52w REPLICATION 1 DEFAULT"
          done
          echo ""
        else
          for x in $arr
